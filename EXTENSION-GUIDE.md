...
### 5.1 开发环境 / Development Environment

- **代码编辑器 / Code Editor**: VS Code
- **调试工具 / Debugging**: Chrome DevTools
- **API 测试 / API Testing**: Postman
- **版本控制 / Version Control**: Git
- **PHP 调试 / PHP Debugging**: Xdebug

### 5.2 VS Code 扩展 / VS Code Extensions
...
```json
{
  "recommendations": [
    ...
    "mermaidtools.vscode-mermaid"    // Mermaid diagrams
  ]
}
```
...
EventBus.prototype.emit = function(event, payload) {
  console.log(`[EventBus] Event: ${event}`, payload);
...
store.subscribe((newState) => {
  console.log('[AppStateStore] State changed:', newState);
});
```
...
describe('JIT Compiler', () => {
  test('should compile system prompt with multiple experts', () => {
    const activeIntents = new Set(['datatree', 'plugins']);
    const prompt = generateSystemPrompt(activeIntents);
    
    expect(prompt).toContain('数据拓扑架构师');
    expect(prompt).toContain('算法生态专家');
