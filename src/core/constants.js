...
/**
 * ThinkMode - 思考模式枚举
 * Think Mode Enumeration
 * 
 * 定义三种推理模式 / Defines three reasoning modes:
 * - THINK_MAX: 深度推理，最高质量 (默认) / Deep reasoning, highest quality (default)
 * - NONTHINK_PRECISE: 禁用推理，精确模式 (temp=0.1, top_p=0.1) / No reasoning, precise mode
 * - NONTHINK_CREATIVE: 禁用推理，发散模式 (temp=0.7, top_p=0.7) / No reasoning, creative mode
 * 
 * 使用方式 / Usage:
 * ```javascript
 * const config = getThinkModeConfig(ThinkMode.NONTHINK_PRECISE);
 * // => { thinking: { type: 'disabled' }, temperature: 0.1, top_p: 0.1 }
 * ```
 */
const ThinkMode = {
  /** 深度推理模式 - 启用 thinking，reasoning_effort=max */
  THINK_MAX: 'think-max',
  
  /** 精确模式 - 禁用 thinking，temperature=0.1，top_p=0.1 */
  NONTHINK_PRECISE: 'nonthink-precise',
  
  /** 发散模式 - 禁用 thinking，temperature=0.7，top_p=0.7 */
  NONTHINK_CREATIVE: 'nonthink-creative'
};
